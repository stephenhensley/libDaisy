...
/** LED driver for one or multiple PCA9635 8bit PWM chips connected to
 * a single I2C peripheral.
 * It includes gamma correction from 8bit brightness values but it
 * can also be supplied with raw 8bit values.
 * This driver uses two buffers - one for drawing, one for transmitting.
 * Multiple LedDriverPca9635 instances can be used at the same time.
 * \param numDrivers    The number of PCA9635 driver attached to the I2C
 *                      peripheral.
 * \param persistentBufferContents If set to true, the current draw buffer
 *                      contents will be copied to the next draw buffer
 *                      during SwapBuffersAndTransmit(). Use this, if you plan
 ...
 */
template <int numDrivers, bool persistentBufferContents = true>
class LedDriverPca9635
{
  public:
    /** Buffer Type for a single PCA9685 driver chip. */
    struct __attribute__((packed)) PCA9635TransmitBuffer
    {
        /** register address */
        uint8_t registerAddr = (0x80) | PCA9635_LED0;
        struct __attribute__((packed))
        {
            /** cycle at which to switch on the led */
            uint8_t val;
        ...
        /** full size in bytes */
        static constexpr uint16_t size = 16 + 1;
    };
    /** Buffer type for the entire DMA buffer. */
    using DmaBuffer = PCA9635TransmitBuffer[numDrivers];
     ...
     * \param dma_buffer_a  The first buffer for the DMA. This must be placed in
     *                      D2 memory by adding the DMA_BUFFER_MEM_SECTION attribute
     *                      like this: `LedDriverPca9635<2>::DmaBuffer DMA_BUFFER_MEM_SECTION bufferA;`
     * \param dma_buffer_b  The second buffer for the DMA. This must be placed in
     *                      D2 memory by adding the DMA_BUFFER_MEM_SECTION attribute
     *                      like this: `LedDriverPca9635<2>::DmaBuffer DMA_BUFFER_MEM_SECTION bufferB;`
     * \param oe_pin        If the output enable pin is used, supply its configuration here.
     *                      It will automatically be pulled low by the driver.
    */
    void Init(I2CHandle i2c,
    ...
    {
        // const uint16_t cycles = gamma_table_[brightness];
        const uint8_t cycles = (uint8_t)(gamma_table_[brightness] >> 4);
        SetLedRaw(ledIndex, cycles);
    }

    /** Sets a single led to a raw 8bit brightness between 0 and 255. */
    void SetLedRaw(int ledIndex, uint8_t rawBrightness)
    {
        const auto d  = GetDriverForLed(ledIndex);
        const auto ch = GetDriverChannelForLed(ledIndex);
        // mask away the "full on" bit
        draw_buffer_[d].leds[ch].val = rawBrightness;
        // const auto on = draw_buffer_[d].leds[ch].val & (0xFF);
        // draw_buffer_[d].leds[ch].off = (on + rawBrightness) & (0x0FFF);
        // full on condition
        // if(rawBrightness >= 0x0FFF)
        //     draw_buffer_[d].leds[ch].on = 0x1000 | on; // set "full on" bit
    ...
    void SwapBuffersAndTransmit()
    {
        ...
        if(persistentBufferContents)
        {
            for(int d = 0; d < numDrivers; d++)
                for(int ch = 0; ch < 16; ch++)
                    draw_buffer_[d].leds[ch].val
                        = transmit_buffer_[d].leds[ch].val;
        }
  ...
  private:
    void ContinueTransmission()
    {
        ...
        const auto    d       = current_driver_idx_;
        const uint8_t address = PCA9635_I2C_BASE_ADDRESS | addresses_[d];
        const auto    status  = i2c_.TransmitDma(address,
                                             (uint8_t*)&transmit_buffer_[d],
                                             PCA9635TransmitBuffer::size,
                                             &TxCpltCallback,
                                             this);
        if(status != I2CHandle::Result::OK)
        {
    ...
    void InitializeBuffers()
    {
        for(int led = 0; led < GetNumLeds(); led++)
        {
            ...
            const auto ch                    = GetDriverChannelForLed(led);
            const auto startCycle            = GetStartCycleForLed(led);
            draw_buffer_[d].registerAddr     = 0x80 | PCA9635_LED0;
            draw_buffer_[d].leds[ch].val     = startCycle;
            transmit_buffer_[d].registerAddr = 0x80 | PCA9635_LED0;
            transmit_buffer_[d].leds[ch].val = startCycle;
        }
    }
    ...
    void InitializeDrivers()
    {
        ...
        for(int d = 0; d < numDrivers; d++)
        {
            const uint8_t address = PCA9635_I2C_BASE_ADDRESS | addresses_[d];
            uint8_t       buffer[2];
            /** Set Mode 1 normal mode no subaddress, no all call, no autoinc */
            buffer[0] = PCA9635_MODE1;
            buffer[1] = 0x00;
            i2c_.TransmitBlocking(address, buffer, 2, 1);
            System::Delay(20);
            ...
            /** Now set LEDout for each LED */
            buffer[0] = PCA9635_LEDOUT0;
            buffer[1] = 0b10101010;
            i2c_.TransmitBlocking(address, buffer, 2, 5);
            buffer[0] = PCA9635_LEDOUT1;
            buffer[1] = 0b10101010;
            i2c_.TransmitBlocking(address, buffer, 2, 5);
            buffer[0] = PCA9635_LEDOUT2;
            buffer[1] = 0b10101010;
            i2c_.TransmitBlocking(address, buffer, 2, 5);
            buffer[0] = PCA9635_LEDOUT3;
            buffer[1] = 0b10101010;
            i2c_.TransmitBlocking(address, buffer, 2, 5);
        }
    }
    ...
    {
        auto drv_ptr = reinterpret_cast<
            LedDriverPca9635<numDrivers, persistentBufferContents>*>(context);
        drv_ptr->ContinueTransmission();
    }

    I2CHandle              i2c_;
    PCA9635TransmitBuffer* draw_buffer_;
    PCA9635TransmitBuffer* transmit_buffer_;
    uint8_t                addresses_[numDrivers];
    Pin                    oe_pin_;
    GPIO                   oe_pin_gpio_;
    // index of the dirver that is currently updated.
    ...
    static constexpr uint8_t PCA9635_I2C_BASE_ADDRESS = 0b00000000;
    static constexpr uint8_t PCA9635_MODE1
        = 0x00; // location for Mode1 register address
    static constexpr uint8_t PCA9635_MODE2
        = 0x01; // location for Mode2 reigster address
    static constexpr uint8_t PCA9635_LED0
        = 0x02; // location for start of LED0 registers
    static constexpr uint8_t PRE_SCALE_MODE
        = 0xFE; //location for setting prescale (clock speed)
    static constexpr uint8_t PCA9635_LEDOUT0
        = 0x14; // driver output state (0-3)
    static constexpr uint8_t PCA9635_LEDOUT1
        = 0x15; // driver output state (4-7)
    static constexpr uint8_t PCA9635_LEDOUT2
        = 0x16; // driver output state (8-11)
    static constexpr uint8_t PCA9635_LEDOUT3
        = 0x17; // driver output state (12-15)
};
